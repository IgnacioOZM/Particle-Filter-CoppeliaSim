...
class ParticleFilter:
    ...
    def resample(self, measurements: List[float]):
        ...
        particle_probabilty = np.zeros(len(self._particles), dtype=object)
        new_particles = np.zeros(self._particles.shape, dtype=object)

        for i in range(0, len(self._particles)):
            particle_probabilty[i] = self._measurement_probability(
                measurements=measurements, particle=self._particles[i])

        n = len(self._particles)
        index = random.randint(0, n - 1)
        beta = 0.0
        w_max = max(particle_probabilty)

        for i in range(n):
            beta += random.uniform(0.0, 2.0 * w_max)

            while beta > particle_probabilty[index]:
                beta -= particle_probabilty[index]
                index = (index + 1) % n
    ...
    def resample_v2(self, measurements: List[float]):
        ...
        for i in range(0, len(self._particles)):
            particle_probabilty[i] = self._measurement_probability(
                measurements=measurements, particle=self._particles[i])
        particle_weigths = particle_probabilty / sum(particle_probabilty)

        neff = self.neff(particle_weigths)
        n = len(self._particles)

        if(neff < n):
            if n > self._minimum_number_of_particles:
                n = max(n - 100, self._minimum_number_of_particles)
            new_particles = np.zeros((n, 3), dtype=object)

            # Systematic Resampling Method
            positions = (self._rng.random() + np.arange(n)) / n
            cumulative_sum = np.cumsum(particle_weigths)
            i, j = 0, 0
            while i < n:
